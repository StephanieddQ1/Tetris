/*
This is the Game class for the tetris game project
Chiamaka Onwude
Alfred Okonkwo
...
public class TetrisGame {
            ...
            for(int row = 0; row < rows; row++){
                for(int col = 0; col < cols; col++){
                    for(int seg = 0; seg < fallingBrick.numSegments; seg++){
                        if(fallingBrick.position[seg][0] == row && 
                                fallingBrick.position[seg][1] == col){
    ...
    private void transferColor(){
        for(int seg = 0; seg < getNumSegs(); seg++)
            background[fallingBrick.position[seg][0] - 1][fallingBrick.position
                    [seg][1]] = getFallingBrickColor();
    ...
    public void saveToFile(String fName){
        ...
        if(fileConnection.exists() && !fileConnection.canWrite()){
            JOptionPane.showMessageDialog(null,"Trouble opening to Write, file: "+fName);
            return;
        }
        try{
            ...
            JOptionPane.showMessageDialog(null,"Trouble writing to file:"+ fName);
        }
    }
    public void retrieveFromFile(){
        try{
            ...
            int choice = JOptionPane.showOptionDialog(null, "Saved Games", "Test",
                    0,0,null, saved_games, saved_games[0]);
            ...
            JOptionPane.showMessageDialog(null,"Trouble opening file to read:   ");
            return;
        }
        catch (Exception e){
            JOptionPane.showMessageDialog(null,"Error occured during read from File");
        }
    }
